        ...
        <div className="relative">
            <button
                ...
                className={isFlatRed
                    ...
                    : isShellBrand
                        ? "w-[264px] h-[41px] flex items-center gap-2 border border-[var(--shell-border)] bg-[var(--shell-side)] hover:bg-[var(--shell-surface)] rounded-xl pl-3 pr-3 transition-all text-[var(--foreground)] shadow-sm"
                    : "flex items-center gap-3 bg-[var(--shell-side)] hover:bg-[var(--shell-border)] border border-[var(--shell-border)] rounded-full pl-2 pr-4 py-1.5 transition-all text-sm font-medium text-[var(--foreground)]"}
            >
                <div className={isFlatRed ? "w-7 h-7 rounded-full border-[3px] border-red-600 bg-white" : isShellBrand ? "w-6.5 h-6.5 rounded-full bg-gradient-to-tr from-pink-500 via-fuchsia-500 to-orange-400 p-[2px]" : "w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 p-[2px]"}>
                    <div className={isFlatRed ? "hidden" : isShellBrand ? "w-full h-full rounded-full bg-zinc-900/90 flex items-center justify-center" : "w-full h-full rounded-full bg-zinc-900 flex items-center justify-center"}>
                        <UserCircleIcon className={isShellBrand ? "w-3.5 h-3.5 text-zinc-400" : "w-5 h-5 text-zinc-400"} />
                    </div>
                </div>
                <div className={isFlatRed ? "flex items-center leading-none flex-1" : isShellBrand ? "flex flex-col items-start leading-none gap-0 flex-1 min-w-0" : "flex flex-col items-start leading-none gap-0.5"}>
                    ...
                    {isShellBrand && <span className="text-[9px] leading-tight uppercase tracking-wider text-zinc-500 font-bold">{selectedNetwork}</span>}
                    {!isFlatRed && !isShellBrand && <span className="text-[9px] text-zinc-500 uppercase tracking-wider">{selected.role}</span>}
                </div>
                <ChevronDownIcon className={`${isFlatRed ? "w-4 h-4 text-zinc-700" : "w-4 h-4 text-zinc-500"} transition-transform ${isOpen ? "rotate-180" : ""}`} />
            </button>
