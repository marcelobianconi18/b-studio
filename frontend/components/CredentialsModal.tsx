        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl">
                <div className="flex items-center gap-3 mb-6">
                    <div className="w-10 h-10 rounded-full bg-white/10/10 flex items-center justify-center">
                        <KeyIcon className="w-5 h-5 text-[var(--foreground)]" />
                    </div>
                    <div>
                        <h2 className="text-lg font-bold text-[var(--foreground)]">Configurar Chaves de API</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="bg-white/10/20 p-4 rounded-xl border border-white/20/30 mb-6">
                        <h3 className="text-[var(--foreground)] text-sm font-bold mb-2">Conexão Automática (Recomendado)</h3>
                        <p className="text-xs text-[var(--foreground)]/70 mb-4">Insira os dados do App Meta para gerar o token automaticamente.</p>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                ...
                                <input
                                    type="text"
                                    className="w-full bg-black border border-zinc-800 rounded-lg px-2 py-1.5 text-xs text-[var(--foreground)]"
                        ...
                        <button
                            ...
                            className="w-full py-2 bg-[#1877F2] hover:bg-[#166fe5] text-[var(--foreground)] font-bold rounded-lg text-sm flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                    ...
                    <div className="border-t border-zinc-800 my-4 pt-4">
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                type="password"
                                className="w-full bg-black border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:border-white/20 focus:outline-none transition-colors placeholder:text-[var(--foreground)]"
                                placeholder="EAAG..."
                    ...
                    <div className="pt-4 flex gap-3">
                        <button
                            type="button"
                            onClick={onClose}
                            className="flex-1 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-[var(--foreground)] font-bold rounded-xl transition-colors text-sm"
                        >
                            Cancelar
                        </button>
                        <button
                            ...
                            className={`flex-1 py-2.5 font-bold rounded-xl transition-colors text-sm flex items-center justify-center gap-2
                                ${success ? "bg-white/10 text-[var(--foreground)]" : "bg-white text-black hover:bg-[var(--accent-primary)]"}
                            `}
                        >
                    ...
                    {error && (
                        <p className="text-xs text-[var(--foreground)] pt-1">{error}</p>
                    )}
                </form>
            </div>
        </div>
