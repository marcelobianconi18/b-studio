...
async def create_campaign():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        params = {
            ...
            "status": "DRAFT",  # Deixa como rascunho para publicar manualmente
            "special_ad_categories": '["POLITICS"]',  # Categoria de tema social
...
async def main():
    ...
    print(f"\n{BLUE}Deseja continuar? (s/n){RESET}")
    response = input("> ").lower()
    
    if response != 's':
        print(f"\n{YELLOW}⚠️  Operação cancelada.{RESET}")
        return
    ...
    adset2_targeting = {
        ...
        "age_max": 65,
        "custom_audiences": [
            {"id": "lookalike_1percent_followers"}  # Placeholder
        ]
    }
    ...
    adset3_targeting = {
        ...
        "age_max": 65,
        "custom_audiences": [
            {"id": "instagram_engagers_365d"}  # Placeholder
        ]
    }
...
#mandatopopular #ProfessorLemos #lemossempresente #lemoseuapoio"""
    ...
    if 'adset1_id' in locals():
        print(f"   ✅ Conjunto 1 (Afetados): {adset1_id}")
    if 'adset2_id' in locals():
        print(f"   ✅ Conjunto 2 (Lookalike): {adset2_id}")
    if 'adset3_id' in locals():
        print(f"   ✅ Conjunto 3 (Engajados): {adset3_id}")
    ...
    print(f"\n   3. Revise:")
    print(f"      ✅ Categoria de tema social")
