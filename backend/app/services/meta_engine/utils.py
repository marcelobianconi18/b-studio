...
def setup_logging():
    ...
    # Allow override for constrained environments
    override_dir = os.environ.get("META_ADS_MCP_LOG_DIR") or os.environ.get("BIA_META_ADS_LOG_DIR")
    if override_dir:
        base_path = pathlib.Path(override_dir)
    else:
    ...
    # Create directory if it doesn't exist (fallback to /tmp on failure)
    log_dir = base_path / "meta-ads-mcp"
    try:
        log_dir.mkdir(parents=True, exist_ok=True)
    except Exception:
        log_dir = pathlib.Path("/tmp") / "meta-ads-mcp"
        log_dir.mkdir(parents=True, exist_ok=True)
