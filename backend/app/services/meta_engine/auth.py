...
def process_token_response(token_container):
    """Process the token response (either code or token) from Facebook."""
    global needs_authentication, auth_manager
    
    if not token_container:
        return False
        
    auth_code = token_container.get('auth_code')
    short_lived_token = token_container.get('token')
    
    if auth_code:
        logger.info(f"Processing authorization code from Facebook OAuth: {auth_code[:10]}...")
        # Exchange the code for an access token
        long_lived_token_info = exchange_code_for_token(auth_code)
        ...
        logger.info("Processing direct token response from Facebook OAuth")
...
def exchange_code_for_token(code: str) -> Optional[TokenInfo]:
    ...
    try:
        ...
        # We need the same redirect URI used in the authorize request
        ...
        if response.status_code == 200:
            ...
            if access_token:
                # If we got a code exchange token, it might already be long-lived, 
                # but if not, we'll try to exchange it again just in case
                # Actually, standard code exchange for business apps often returns 
                # a long-lived user token directly or we need one more hop.
                
                # Check if it looks like a long-lived token (usually > 50 days)
                if expires_in and expires_in > 4000000: # ~46 days
                    return TokenInfo(access_token=access_token, expires_in=expires_in)
                
                # Try to exchange for a long-lived one just to be sure
                long_lived = exchange_token_for_long_lived(access_token)
                if long_lived:
                    return long_lived
                return TokenInfo(access_token=access_token, expires_in=expires_in)
        else:
