...
class CallbackHandler(BaseHTTPRequestHandler):
    ...
    def _handle_oauth_callback(self):
        ...
        # Check if we're being redirected from Facebook with an authorization code or token
        parsed_url = urlparse(self.path)
        params = parse_qs(parsed_url.query)
        
        # Check for code or token parameter
        ...
        if error:
            ...
            if code:
                ...
                logger.info(f"Received access token: {token[:10]}...")
                token_container.update({
                    "token": token,
            ...
            # Check for fragment (Facebook Implicit Grant returns token in the fragment)
            html = """
            <html>
            <head><title>Processing Authorization...</title></head>
            <body style="font-family: sans-serif; text-align: center; padding: 50px;">
                ...
                <script>
                    ...
                    if (window.location.hash) {
                        ...
                        if (token) {
                            // Redirect to the same URL but with the token in the query string
                            // so the server can see it
                            window.location.href = window.location.origin + window.location.pathname + 
                                '?token=' + encodeURIComponent(token) + 
                                (expiresIn ? '&expires_in=' + encodeURIComponent(expiresIn) : '') +
                                (state ? '&state=' + encodeURIComponent(state) : '');
                        } else {
                            document.body.innerHTML = '<h1 style="color: #e53e3e;">Unexpected Response</h1><p>No authorization data found. Please try again.</p>';
                        }
                    } else {
                        document.body.innerHTML = '<h1 style="color: #e53e3e;">Unexpected Response</h1><p>No authorization code or token received. Please try again.</p>';
                    }
                </script>
            </body>
            </html>
            """
            logger.warning("OAuth callback received without code - waiting for client-side fragment capture")
