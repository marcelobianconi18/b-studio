#!/usr/bin/env python3
"""
Criador e Publicador de Campanha - Professor Lemos - Pedágio Injusto
VERSÃO AUTOMÁTICA - CRIA E PUBLICA IMEDIATAMENTE
...
def print_header():
    ...
    print(f"{CYAN}Período:{RESET} {START_DATE} a {END_DATE}")
    print(f"{CYAN}Status:{RESET} PUBLICAR IMEDIATAMENTE")
    print("=" * 80)
...
async def create_campaign():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        params = {
            ...
            "status": "ACTIVE",  # ATIVA IMEDIATAMENTE
...
async def create_ad(adset_id, name, page_id, instagram_id, video_url, ad_copy, headline):
    """Cria um anúncio."""
    async with httpx.AsyncClient(timeout=30.0) as client:
...
async def main():
    ...
    print(f"{BOLD}Status:{RESET} PUBLICAR IMEDIATAMENTE")
    print(f"{BOLD}Orçamento:{RESET} R$ {BUDGET_TOTAL:.2f} total")
    print(f"{BOLD}Período:{RESET} {START_DATE} a {END_DATE}")
    
    print(f"\n{YELLOW}⚠️  ATENÇÃO:{RESET}")
    print(f"   A campanha será CRIADA e PUBLICADA imediatamente")
    print(f"   Link: https://adsmanager.facebook.com/adsmanager/manage/campaigns?act={AD_ACCOUNT_ID.replace('act_', '')}")
    
    print(f"\n{BLUE}Iniciando criação e publicação...{RESET}")
    ...
    print(f"\n{GREEN}✅ Campanha criada e ATIVADA!{RESET}")
    print(f"   ID: {campaign_id}")
    print(f"   Nome: {CAMPAIGN_NAME}")
    print(f"   Status: ACTIVE (No Ar)")
    ...
    if 'error' in adset1_result:
        ...
        print(f"{GREEN}✅ Conjunto 1 criado e ATIVADO!{RESET}")
    ...
    if 'error' in adset2_result:
        ...
        print(f"{GREEN}✅ Conjunto 2 criado e ATIVADO!{RESET}")
    ...
    if 'error' in adset3_result:
        ...
        print(f"{GREEN}✅ Conjunto 3 criado e ATIVADO!{RESET}")
...
#mandatopopular #ProfessorLemos #lemossempresente #lemoseuapoio"""
    ...
    if adset1_id:
        ...
        if 'error' in ad1_result:
            ...
            print(f"{GREEN}✅ Anúncio 1 criado e ATIVADO!{RESET}")
            print(f"   ID: {ad1_id}")
    
    # Resumo Final
    print_section(f"📋 RESUMO FINAL")
    
    print(f"\n{GREEN}✅ CAMPANHA CRIADA E PUBLICADA COM SUCESSO!{RESET}")
    print(f"\n{BOLD}Campanha:{RESET}")
    print(f"   ID: {campaign_id}")
    print(f"   Nome: {CAMPAIGN_NAME}")
    print(f"   Status: ACTIVE (No Ar)")
    ...
    print_section(f"🚀 CAMPANHA NO AR!")
    
    print(f"\n{GREEN}✅ SUA CAMPANHA ESTÁ RODANDO AGORA!{RESET}")
    print(f"\n{BOLD}Links úteis:{RESET}")
    print(f"\n   📊 Ads Manager:")
    print(f"      https://adsmanager.facebook.com/adsmanager/manage/campaigns?act={AD_ACCOUNT_ID.replace('act_', '')}")
    print(f"\n   📊 Campanha:")
    print(f"      https://adsmanager.facebook.com/adsmanager/manage/campaigns/{campaign_id}")
    print(f"\n   📱 Página:")
    print(f"      https://www.facebook.com/{PAGE_ID}")
    print(f"\n   📷 Instagram:")
    print(f"      https://www.instagram.com/{INSTAGRAM_ID}")
    print(f"\n   🎥 Vídeo:")
    print(f"      {VIDEO_URL}")
    
    print(f"\n{YELLOW}⏱️  Cronograma:{RESET}")
    print(f"   • Início: {START_DATE}")
    print(f"   • Término: {END_DATE}")
    print(f"   • Orçamento: R$ {BUDGET_TOTAL:.2f}")
    
    print(f"\n{BLUE}📊 Monitore os resultados no Ads Manager!{RESET}")
    print(f"\n{CYAN}{'=' * 80}{RESET}")
    print(f"{BOLD}CAMPANHA ATIVA E RODANDO!{RESET}")
